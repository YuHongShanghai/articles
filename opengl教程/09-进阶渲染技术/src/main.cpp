...
int main()
{
    ...
    GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT,
                                          "09 - Advanced Rendering", NULL, NULL);
    if (!window)
    {
    ...
    unsigned int floorTexture = loadTexture("resources/floor.png");
...
void processInput(GLFWwindow* window)
{
    ...
    // 数字键 0-5 切换后处理效果
    if (glfwGetKey(window, GLFW_KEY_0) == GLFW_PRESS) currentEffect = 0;
    if (glfwGetKey(window, GLFW_KEY_1) == GLFW_PRESS) currentEffect = 1;
    if (glfwGetKey(window, GLFW_KEY_2) == GLFW_PRESS) currentEffect = 2;
    if (glfwGetKey(window, GLFW_KEY_3) == GLFW_PRESS) currentEffect = 3;
    if (glfwGetKey(window, GLFW_KEY_4) == GLFW_PRESS) currentEffect = 4;
    if (glfwGetKey(window, GLFW_KEY_5) == GLFW_PRESS) currentEffect = 5;
}
