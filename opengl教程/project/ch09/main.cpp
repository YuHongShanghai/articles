...
// 生成 8x8 棋盘格占位纹理（文件缺失时使用）
static unsigned int makeFallbackTexture()
{
    const int SIZE = 8;
    ...
    for (int y = 0; y < SIZE; y++) {
        for (int x = 0; x < SIZE; x++) {
            bool white = ((x + y) % 2 == 0);
            int idx = (y * SIZE + x) * 3;
            pixels[idx + 0] = white ? 200 : 80;   // R
            pixels[idx + 1] = white ? 200 : 80;   // G
...
unsigned int loadTexture(const char *path)
{
    ...
    {
        std::cout << "Texture failed to load (using fallback checkerboard): "
                  << path << std::endl;
        stbi_image_free(data);
...
// 生成纯色立方体贴图占位面（天蓝色渐变，区分6个面）
static void makeFallbackCubemapFace(unsigned int faceIndex)
{
    // 6个面各用不同的蓝/青色调，视觉上有区分
    static const unsigned char faceColors[6][3] = {
        {100, 160, 220},  // right  - 天蓝
        {100, 160, 220},  // left   - 天蓝
        { 80, 120, 200},  // top    - 深蓝
        { 60,  80,  60},  // bottom - 深绿（地面）
        {100, 160, 220},  // front  - 天蓝
        {100, 160, 220},  // back   - 天蓝
    };
    const int SIZE = 4;
    unsigned char pixels[SIZE * SIZE * 3];
    const unsigned char *c = faceColors[faceIndex];
    ...
    glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + faceIndex,
                 0, GL_RGB, SIZE, SIZE, 0, GL_RGB, GL_UNSIGNED_BYTE, pixels);
}
...
unsigned int loadCubemap(const std::vector<std::string> &faces)
{
    ...
    for (unsigned int i = 0; i < faces.size(); i++)
    {
        ...
        {
            std::cout << "Cubemap texture failed to load (using fallback): "
                      << faces[i] << std::endl;
            stbi_image_free(data);
            makeFallbackCubemapFace(i);
        }
    }
